(function () {
    ...
    function setupArticlePath() {
        ...
        const exactTarget = exactArticleTarget(article);
        const contextualTarget = (pathConfig && pathConfig.primary) || exactTarget || directoryTarget(intent);
        const contextualPath = new URL(contextualTarget.href, window.location.origin).pathname;
        ...
        const secondary = intent === 'speech'
            ? exactTarget && exactTarget.href !== contextualTarget.href
                ? { ...exactTarget, target: 'exact_speech_model' }
                : { href: '/tts-list', label: 'Compare all local speech models', target: 'tts_directory' }
            : intent === 'app' || contextualIsMacApp
                ? finderTarget
                : contextualTarget;
        ...
        if ('IntersectionObserver' in window) {
            const observer = new IntersectionObserver((entries) => {
                ...
                track('article_cta_view', { article: window.location.pathname, intent, platform, placement: 'before_first_section' });
                observer.disconnect();
            }, { threshold: 0.45 });
