...
function applyLightTheme(html) {
  return html
    ...
    .replace('<link rel="stylesheet" href="/css/amazon-clicks-20260921.css"><script defer src="/js/amazon-clicks-20260921.js"></script></head>', `<style>${scopeLightCss(lightStyle)}</style><link rel="stylesheet" href="/css/amazon-clicks-20260921.css"><script defer src="/js/amazon-clicks-20260921.js"></script></head>`);
}
...
function buy(mac) {
  ...
  return `/go/amazon?q=${encodeURIComponent(query)}&family=computers&product=${encodeURIComponent(mac.id)}&source=hardware_guide`;
}
...
fs.writeFileSync(path.join(out, 'index.html'), `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Mac hardware guides for local AI | LocalClaw</title><meta name="description" content="Current Mac hardware guides for local AI, including Mac mini M6, M5 Pro, Mac Studio M5 Max and M5 Ultra, with realistic memory fit."><meta name="robots" content="index, follow"><link rel="canonical" href="${BASE}/hardware/"><link rel="alternate" type="text/plain" href="${BASE}/llms.txt" title="LocalClaw AI-readable index">${tracking}<style>${style}</style><link rel="stylesheet" href="/css/amazon-clicks-20260921.css"><script defer src="/js/amazon-clicks-20260921.js"></script></head><body>${nav()}<main class="wrap"><header class="hero" style="grid-template-columns:1fr;padding-bottom:36px"><div><h1 class="title">Mac hardware guides for local AI</h1><p class="desc">Choose a Mac and see current local LLM recommendations filtered for its unified memory.</p><p class="updated">All recommendations updated ${UPDATED}</p></div></header><section class="answer-card"><span class="badge">New Mac guide</span><h2 style="margin-top:16px">M6, M5 Pro, M5 Max and M5 Ultra for local AI</h2><p class="section-copy">Compare every new desktop configuration, current M5 MacBooks, Apple-confirmed specifications and LocalClaw's memory-fit recommendations in one source-backed guide.</p><div class="cta"><a class="btn" href="/hardware/new-macs-local-ai">Open the new Mac local AI guide</a></div></section>${guideLinks}</main></body></html>`);
