...
if (multimodalModels.length !== 75) errors.push(`Multimodal model count is ${multimodalModels.length}, expected 75`);
...
if (allSpeechRecords.length !== 73) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 73 preserved routes`);
...
if (localSpeechRecords.length !== 70) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 70`);
...
if (speechModels.length !== 70 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 70 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 74) errors.push('tts/ must contain 73 speech pages plus one index');
