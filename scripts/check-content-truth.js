...
if (uniqueLocalModels.length !== 220) errors.push(`Local LLM route count is ${uniqueLocalModels.length}, expected 220 preserved routes`);
if (indexableLocalModels.length !== 214) errors.push(`Indexable local LLM count is ${indexableLocalModels.length}, expected 214`);
if (unavailableLlmIds.size !== 6) errors.push(`Unavailable LLM tombstone count is ${unavailableLlmIds.size}, expected 6`);
if (multimodalModels.length !== 49) errors.push(`Multimodal model count is ${multimodalModels.length}, expected 49`);
...
if (allSpeechRecords.length !== 60) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 60 preserved routes`);
...
if (localSpeechRecords.length !== 57) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 57`);
...
if (speechModels.length !== 57 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 57 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'models')).filter(file => file.endsWith('.html')).length !== 222) errors.push('models/ must contain 221 model pages plus one index');
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 61) errors.push('tts/ must contain 60 speech pages plus one index');
...
const expectedLatestIds = ['ling-3.0-tiny', 'muse-glimmer-30b', 'qwen3.8-27b', 'deepseek-v4-flash-0731', 'lfm2-5-2-6b'];
...
const fallbackOrder = ['ling-3.0-tiny', 'muse-glimmer-30b', 'qwen3.8-27b'].map(id => newPage.indexOf(`href="/models/${id}"`));
...
if (!index.includes('js/data.js?v=20260818a') || !index.includes('js/app-20260816a.js?v=20260818a')) {
