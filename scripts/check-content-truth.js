...
if (allSpeechRecords.length !== 96) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 96 preserved routes`);
...
if (localSpeechRecords.length !== 93) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 93`);
...
if (speechModels.length !== 93 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 93 verified-local source records');
}
