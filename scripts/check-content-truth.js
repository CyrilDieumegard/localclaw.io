...
if (multimodalModels.length !== 53) errors.push(`Multimodal model count is ${multimodalModels.length}, expected 53`);
...
if (allSpeechRecords.length !== 61) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 61 preserved routes`);
...
if (localSpeechRecords.length !== 58) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 58`);
...
if (speechModels.length !== 58 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 58 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 62) errors.push('tts/ must contain 61 speech pages plus one index');
