...
if (allSpeechRecords.length !== 80) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 80 preserved routes`);
...
if (localSpeechRecords.length !== 77) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 77`);
...
if (speechModels.length !== 77 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 77 verified-local source records');
}
...
if (!index.includes('js/data.js?v=20260901c') || !index.includes('js/app-20260816a.js?v=20260901c')) {
