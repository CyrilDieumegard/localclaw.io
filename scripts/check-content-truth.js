...
if (multimodalModels.length !== 86) errors.push(`Multimodal model count is ${multimodalModels.length}, expected 86`);
...
if (allSpeechRecords.length !== 77) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 77 preserved routes`);
...
if (localSpeechRecords.length !== 74) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 74`);
...
if (speechModels.length !== 74 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 74 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 78) errors.push('tts/ must contain 77 speech pages plus one index');
