...
if (allSpeechRecords.length !== 94) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 94 preserved routes`);
...
if (localSpeechRecords.length !== 91) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 91`);
...
if (speechModels.length !== 91 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 91 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 95) errors.push('tts/ must contain 94 speech pages plus one index');
