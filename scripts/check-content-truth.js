...
if (allSpeechRecords.length !== 86) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 86 preserved routes`);
...
if (localSpeechRecords.length !== 83) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 83`);
...
if (speechModels.length !== 83 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 83 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 87) errors.push('tts/ must contain 86 speech pages plus one index');
