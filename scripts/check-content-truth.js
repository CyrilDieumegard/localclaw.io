...
if (uniqueLocalModels.length !== 237) errors.push(`Local LLM route count is ${uniqueLocalModels.length}, expected 237 canonical routes`);
if (indexableLocalModels.length !== 231) errors.push(`Indexable local LLM count is ${indexableLocalModels.length}, expected 231`);
...
if (allSpeechRecords.length !== 89) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 89 preserved routes`);
...
if (localSpeechRecords.length !== 86) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 86`);
...
if (speechModels.length !== 86 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 86 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'models')).filter(file => file.endsWith('.html')).length !== 239) errors.push('models/ must contain 239 HTML files');
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 90) errors.push('tts/ must contain 89 speech pages plus one index');
...
const expectedLatestIds = ['minicpm5-2b', 'spark-x2-5-4b', 'spark-x2-5-1-7b', 'ibnsina-1.5b', 'k2-horizon-0-9b', 'k2-horizon-mova-36b-a4b', 'deepseek-v4-flash-vision-exp', 'granite4.2-8b', 'granite4.2-30b', 'granite4.2-3b', 'glm-5.3-flash', 'ornith-1-5-9b'];
...
const fallbackOrder = ['minicpm5-2b', 'spark-x2-5-4b', 'spark-x2-5-1-7b'].map(id => newPage.indexOf(`href="/models/${id}"`));
...
if (!index.includes('js/data.js?v=20260908a') || !index.includes('js/app-20260816a.js?v=20260908a')) {
...
const expectedHfStateCounts = {publicGguf: 191, publicModelCard: 40, gated: 4, unavailable: 6};
