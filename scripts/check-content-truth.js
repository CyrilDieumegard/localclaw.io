...
if (allSpeechRecords.length !== 90) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 90 preserved routes`);
...
if (localSpeechRecords.length !== 87) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 87`);
...
if (speechModels.length !== 87 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 87 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 91) errors.push('tts/ must contain 90 speech pages plus one index');
