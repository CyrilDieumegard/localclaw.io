...
if (allSpeechRecords.length !== 97) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 97 preserved routes`);
...
if (localSpeechRecords.length !== 94) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 94`);
...
if (speechModels.length !== 94 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 94 verified-local source records');
}
