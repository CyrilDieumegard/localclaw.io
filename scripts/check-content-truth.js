...
if (uniqueLocalModels.length !== 233) errors.push(`Local LLM route count is ${uniqueLocalModels.length}, expected 233 preserved routes`);
...
if (allSpeechRecords.length !== 85) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 85 preserved routes`);
...
if (localSpeechRecords.length !== 82) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 82`);
...
if (speechModels.length !== 82 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 82 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 86) errors.push('tts/ must contain 85 speech pages plus one index');
...
const expectedLatestIds = ['spark-x2-5-4b', 'spark-x2-5-1-7b', 'ibnsina-1.5b', 'granite4.2-8b', 'granite4.2-30b', 'granite4.2-3b', 'ornith-1-5-9b', 'ornith-1-5-35b-a3b', 'dfm-mimir', 'llm-jp-4-33b-thinking', 'nemotron-3-5-lightning-30b-a3b', 'lfm2-5-vl-3b'];
