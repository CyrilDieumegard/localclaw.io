...
if (allSpeechRecords.length !== 69) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 69 preserved routes`);
...
if (localSpeechRecords.length !== 66) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 66`);
...
if (speechModels.length !== 66 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 66 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 70) errors.push('tts/ must contain 69 speech pages plus one index');
