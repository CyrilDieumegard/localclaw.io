...
if (allSpeechRecords.length !== 91) errors.push(`Speech source count is ${allSpeechRecords.length}, expected 91 preserved routes`);
...
if (localSpeechRecords.length !== 88) errors.push(`Local speech source count is ${localSpeechRecords.length}, expected 88`);
...
if (speechModels.length !== 88 || speechModels.some(model => !localSpeechRecords.some(source => source.id === model.id))) {
  errors.push('Homepage speech export must contain exactly the 88 verified-local source records');
}
...
if (fs.readdirSync(path.join(ROOT, 'tts')).filter(file => file.endsWith('.html')).length !== 92) errors.push('tts/ must contain 91 speech pages plus one index');
