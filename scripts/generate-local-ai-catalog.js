...
function landingPage(key, config) {
  const count = multimodal.filter((model) => model.category === key).length;
  const hasVisualExamples = ['video', '3d'].includes(key);
  const schema = {
...
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
  ...
  ${siteNavAssets()}<link rel="stylesheet" href="/css/local-ai-catalog.css?v=20260816b">${hasVisualExamples ? '<link rel="stylesheet" href="/css/external-media.css?v=20260816a">' : ''}${tracking()}<script type="application/ld+json">${JSON.stringify(schema)}</script>
...
${hasVisualExamples ? '<script src="/js/external-media-catalog.js?v=20260816a"></script><script src="/js/external-media.js?v=20260816b"></script>' : ''}<script src="/js/local-ai-catalog.js?v=20260816b"></script><script src="/js/local-ai-catalog-app.js?v=20260816c"></script></body></html>`;
}
...
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
  ...
  <link rel="icon" href="/images/crab-logo.png"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@500;700&family=Space+Grotesk:wght@500;600;700&display=swap" rel="stylesheet">
  ${siteNavAssets()}<link rel="stylesheet" href="/css/local-ai-catalog.css?v=20260816b"><link rel="stylesheet" href="/css/external-media.css?v=20260816a">${tracking()}<script type="application/ld+json">${JSON.stringify(schema)}</script>
...
<script src="/js/external-media-catalog.js?v=20260816a"></script><script src="/js/external-media.js?v=20260816a"></script><script src="/js/local-ai-search-index.js?v=20260816b"></script><script src="/js/local-ai-catalog-app.js?v=20260816c"></script></body></html>`;
}

function detailPage(model) {
  const config = categoryConfig[model.category];
  const hasVisualExample = ['video', '3d'].includes(model.category);
  ...
  <link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@500;700&family=Space+Grotesk:wght@500;600;700&display=swap" rel="stylesheet">${siteNavAssets()}<link rel="stylesheet" href="/css/local-ai-catalog.css?v=20260816b">${hasVisualExample ? '<link rel="stylesheet" href="/css/external-media.css?v=20260816a">' : ''}${tracking()}<script type="application/ld+json">${JSON.stringify(schema)}</script>
  ...
  <div class="lc-ai-detail-grid"><div>${hasVisualExample ? `<section class="lc-ai-detail-panel"><h2>Official example</h2><p>Load a visual example from the official project source. The media stays on the source server and is never hosted by LocalClaw.</p><div class="lc-external-media" data-external-media data-media-category="${esc(model.category)}" data-media-id="${esc(model.id)}"></div></section>` : ''}<section class="lc-ai-detail-panel"><h2>What it does</h2><div class="lc-ai-task-list">${model.tasks.map((task) => `<span class="lc-ai-task">${esc(prettyTerm(task))}</span>`).join('')}</div><p>${esc(model.hardware_note)}</p><div class="lc-ai-source-note">Hardware figures are practical entry floors, not performance guarantees. Resolution, duration, precision, offloading and runtime versions can materially change memory use.</div></section>
...
</div></main>${footer()}${hasVisualExample ? '<script src="/js/external-media-catalog.js?v=20260816a"></script><script src="/js/external-media.js?v=20260816b"></script>' : ''}</body></html>`;
}
