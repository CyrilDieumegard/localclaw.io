...
if (html !== null) {
  ...
  const itemListNode = graph.find(node => hasType(node, 'ItemList'));
  ...
  const rankingTable = html.match(/<table\b[^>]*class=["'][^"']*\batlas-ranking\b[^"']*["'][^>]*>([\s\S]*?)<\/table>/i);
  if (!rankingTable) {
    issue('Rendered top-20 ranking table is missing');
  } else {
    const tbody = rankingTable[1].match(/<tbody\b[^>]*>([\s\S]*?)<\/tbody>/i);
...
console.log('Local AI Activity Index validation passed: canonical page, honest interest boundary, 3,337 signals across 113 regions, top 20, structured data, controls, assets and sitemap verified.');
